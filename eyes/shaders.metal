...
// Shade a texture for camera view (grayscale from Y channel)
fragment half4 planeFragmentShaderCamera(ColorInOut in [[stage_in]], texture2d<float, access::sample> textureY [[ texture(0) ]])
{
    constexpr sampler colorSampler(address::clamp_to_edge, filter::linear);
    float4 s = textureY.sample(colorSampler, in.texCoord);
    
    // Convert Y channel to RGB grayscale
    half y = half(s.r);
    return half4(y, y, y, 1.0h);
}
