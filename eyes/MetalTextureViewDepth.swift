...
final class CoordinatorCamera: MTKCoordinator {
    init(cameraContent: MetalTextureContent) {
        super.init(content: cameraContent)
    }
    override func prepareFunctions() {
        guard let metalDevice = mtkView.device else { fatalError("Expected a Metal device.") }
        do {
            ...
            pipelineDescriptor.depthAttachmentPixelFormat = .depth32Float
            pipelineDescriptor.vertexFunction = library.makeFunction(name: "planeVertexShader")
            pipelineDescriptor.fragmentFunction = library.makeFunction(name: "planeFragmentShaderCamera")
            pipelineDescriptor.vertexDescriptor = createPlaneMetalVertexDescriptor()
            pipelineState = try metalDevice.makeRenderPipelineState(descriptor: pipelineDescriptor)
        } catch {
            print("Unexpected error: \(error).")
...
struct MetalTextureViewCamera: UIViewRepresentable {
    var content: MetalTextureContent
    
    func makeCoordinator() -> CoordinatorCamera {
        CoordinatorCamera(cameraContent: content)
    }
    
    func makeUIView(context: UIViewRepresentableContext<MetalTextureViewCamera>) -> MTKView {
        let mtkView = MTKView()
        mtkView.delegate = context.coordinator
        mtkView.backgroundColor = context.environment.colorScheme == .dark ? .black : .white
        context.coordinator.setupView(mtkView: mtkView)
    ...
    func updateUIView(_ uiView: MTKView, context: UIViewRepresentableContext<MetalTextureViewCamera>) {
